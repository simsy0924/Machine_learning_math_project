...
각 출력은 기존 행 순서대로 누적합니다. 곱셈→뺄셈 결합은 전체 학습 비교에서
속도 저하가 확인되어 적용하지 않습니다.
...
`presentation/build.mjs`가 만드는 발표 단계 파일을 실제 페이지에서 하나씩 불러와 `계산`을 실행하고,
모든 블록이 오류 없이 계산되는지와 `값 보기` 블록이 생성기가 예측한 값(진리표, 학습 후 손실,
탐색 시도 횟수 등)을 내는지 검사합니다. 디스크의 파일이 생성기와 다르면 실패하므로, 발표 파일을
고칠 때는 `build.mjs`를 고치고 다시 생성한 뒤 이 검사를 돌리세요. 발표 모드 전환 로직 자체는
`bench/presentation-check.mjs`가 검사합니다.
